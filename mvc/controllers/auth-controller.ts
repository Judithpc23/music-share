...
import { toDbUser } from "@/mvc/models/supabase-mappers"
import type { ProfilePrivacity, User } from "@/mvc/models/types"
...
export async function registerWithEmailPassword(input: SignUpInput): Promise<{
  ...
  if (createdUser) {
    ...
    const { error: profileError } = await supabase.from("users").upsert(toDbUser(profile))
    if (profileError) {
